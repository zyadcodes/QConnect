...
export default class FirebaseFunctions {
	...
	// firebase authentication (will also sign the user in). The method will then return the ID that is returned by
	// Firebase Auth
	...
	static async saveTeacherCustomImprovementTags(teacherID, evaluationImprovementTags) {
		try {
	...
	static async updateStudentAssignmentStatus(classID, studentID, status, index) {
		...
		let oldStatus = arrayOfStudents[studentIndex].currentAssignments[index].isReadyEnum;
	...
	static async submitRecordingAudio(file, studentID, classID, assignmentIndex) {
		try {
			...
			if (
				...
				arrayOfStudents[studentIndex].currentAssignments[assignmentIndex] === undefined
			) {
			...
			arrayOfStudents[studentIndex].currentAssignments[assignmentIndex].submission = {
				audioFileID,
				sent,
			};
	...
	static async updateClassAssignment(
		...
		arrayOfStudents.forEach((student) => {
			if (student.currentAssignments === undefined || student.currentAssignments.length === 0) {
	...
	static async completeCurrentAssignment(classID, studentID, evaluationDetails) {
		try {
			...
			let indexOfAssignment = arrayOfStudents[studentIndex].currentAssignments.findIndex(
				(element) => {
					return (
			...
			if (sendNotification) {
				...
				currentClass.teachers.forEach(async (teacherID) => {
					...
					this.functions.httpsCallable('sendNotification')({
						topic: teacherID,
						title: strings.StudentUpdate,
						body: arrayOfStudents[studentIndex].name + ' ' + strings.PracticeLogNotification,
					});
				});
			}
		} catch (err) {
